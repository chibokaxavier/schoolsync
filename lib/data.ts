// TEMPORARY DATA

// role is now managed dynamically via AuthContext

export const teachersData = [
  {
    id: 1,
    teacherId: "1234564890",
    name: "John Doe",
    email: "john@doe.com",
    photo:
      "https://images.pexels.com/photos/2888150/pexels-photo-2888150.jpeg?auto=compress&cs=tinysrgb&w=1200",
    phone: "1234564890",
    subjects: ["Math", "Geometry"],
    classes: ["1B", "2A", "3C"],
    address: "123 Main St, Anytown, USA",
    status: "Active",
  },
  {
    id: 2,
    teacherId: "1234564890",
    name: "Jane Doe",
    email: "jane@doe.com",
    photo:
      "https://images.pexels.com/photos/936126/pexels-photo-936126.jpeg?auto=compress&cs=tinysrgb&w=1200",
    phone: "1234564890",
    subjects: ["Physics", "Chemistry"],
    classes: ["5A", "4B", "3C"],
    address: "123 Main St, Anytown, USA",
    status: "Active",
  },
  {
    id: 3,
    teacherId: "1234564890",
    name: "Mike Geller",
    email: "mike@geller.com",
    photo:
      "https://images.pexels.com/photos/428328/pexels-photo-428328.jpeg?auto=compress&cs=tinysrgb&w=1200",
    phone: "1234564890",
    subjects: ["Biology"],
    classes: ["5A", "4B", "3C"],
    address: "123 Main St, Anytown, USA",
    status: "Suspended",
  },
  {
    id: 4,
    teacherId: "1234564890",
    name: "Jay French",
    email: "jay@gmail.com",
    photo:
      "https://images.pexels.com/photos/118465/pexels-photo-118465.jpeg?auto=compress&cs=tinysrgb&w=1200",
    phone: "1234564890",
    subjects: ["History"],
    classes: ["5A", "4B", "3C"],
    address: "123 Main St, Anytown, USA",
    status: "Active",
  },
  {
    id: 5,
    teacherId: "1234564890",
    name: "Jane Smith",
    email: "jane@gmail.com",
    photo:
      "https://images.pexels.com/photos/1102341/pexels-photo-1102341.jpeg?auto=compress&cs=tinysrgb&w=1200",
    phone: "1234564890",
    subjects: ["Music", "History"],
    classes: ["5A", "4B", "3C"],
    address: "123 Main St, Anytown, USA",
    status: "Active",
  },
  {
    id: 6,
    teacherId: "1234567890",
    name: "Anna Santiago",
    email: "anna@gmail.com",
    photo:
      "https://images.pexels.com/photos/712513/pexels-photo-712513.jpeg?auto=compress&cs=tinysrgb&w=1200",
    phone: "1234567890",
    subjects: ["Physics"],
    classes: ["5A", "4B", "3C"],
    address: "123 Main St, Anytown, USA",
    status: "Active",
  },
  {
    id: 7,
    teacherId: "1234567890",
    name: "Allen Black",
    email: "allen@black.com",
    photo:
      "https://images.pexels.com/photos/1438081/pexels-photo-1438081.jpeg?auto=compress&cs=tinysrgb&w=1200",
    phone: "1234567890",
    subjects: ["English", "Spanish"],
    classes: ["5A", "4B", "3C"],
    address: "123 Main St, Anytown, USA",
    status: "Active",
  },
  {
    id: 8,
    teacherId: "1234567890",
    name: "Ophelia Castro",
    email: "ophelia@castro.com",
    photo:
      "https://images.pexels.com/photos/1036623/pexels-photo-1036623.jpeg?auto=compress&cs=tinysrgb&w=1200",
    phone: "1234567890",
    subjects: ["Math", "Geometry"],
    classes: ["5A", "4B", "3C"],
    address: "123 Main St, Anytown, USA",
    status: "Active",
  },
  {
    id: 9,
    teacherId: "1234567890",
    name: "Derek Briggs",
    email: "derek@briggs.com",
    photo:
      "https://images.pexels.com/photos/842980/pexels-photo-842980.jpeg?auto=compress&cs=tinysrgb&w=1200",
    phone: "1234567890",
    subjects: ["Literature", "English"],
    classes: ["5A", "4B", "3C"],
    address: "123 Main St, Anytown, USA",
    status: "Active",
  },
  {
    id: 10,
    teacherId: "1234567890",
    name: "John Glover",
    email: "john@glover.com",
    photo:
      "https://images.pexels.com/photos/1043474/pexels-photo-1043474.jpeg?auto=compress&cs=tinysrgb&w=1200",
    phone: "1234567890",
    subjects: ["Biology"],
    classes: ["5A", "4B", "3C"],
    address: "123 Main St, Anytown, USA",
    status: "Active",
  },
];

export const studentsData = [
  {
    id: 1,
    studentId: "1234567890",
    name: "John Doe",
    email: "john@doe.com",
    photo:
      "https://images.pexels.com/photos/2888150/pexels-photo-2888150.jpeg?auto=compress&cs=tinysrgb&w=1200",
    phone: "1234567890",
    grade: 7,
    class: "1B",
    address: "123 Main St, Anytown, USA",
    status: "Active",
  },
  {
    id: 2,
    studentId: "1234567890",
    name: "Jane Doe",
    email: "jane@doe.com",
    photo:
      "https://images.pexels.com/photos/936126/pexels-photo-936126.jpeg?auto=compress&cs=tinysrgb&w=1200",
    phone: "1234567890",
    grade: 8,
    class: "5A",
    address: "123 Main St, Anytown, USA",
    status: "Active",
  },
  {
    id: 3,
    studentId: "1234567890",
    name: "Mike Geller",
    email: "mike@geller.com",
    photo:
      "https://images.pexels.com/photos/428328/pexels-photo-428328.jpeg?auto=compress&cs=tinysrgb&w=1200",
    phone: "1234567890",
    grade: 9,
    class: "5A",
    address: "123 Main St, Anytown, USA",
    status: "Suspended",
  },
  {
    id: 4,
    studentId: "1234567890",
    name: "Jay French",
    email: "jay@gmail.com",
    photo:
      "https://images.pexels.com/photos/1187765/pexels-photo-1187765.jpeg?auto=compress&cs=tinysrgb&w=1200",
    phone: "1234567890",
    grade: 10,
    class: "5A",
    address: "123 Main St, Anytown, USA",
    status: "Active",
  },
  {
    id: 5,
    studentId: "1234567890",
    name: "Jane Smith",
    email: "jane@gmail.com",
    photo:
      "https://images.pexels.com/photos/1102341/pexels-photo-1102341.jpeg?auto=compress&cs=tinysrgb&w=1200",
    phone: "1234567890",
    grade: 11,
    class: "5A",
    address: "123 Main St, Anytown, USA",
    status: "Active",
  },
  {
    id: 6,
    studentId: "1234567890",
    name: "Anna Santiago",
    email: "anna@gmail.com",
    photo:
      "https://images.pexels.com/photos/712513/pexels-photo-712513.jpeg?auto=compress&cs=tinysrgb&w=1200",
    phone: "1234567890",
    grade: 12,
    class: "5A",
    address: "123 Main St, Anytown, USA",
    status: "Active",
  },
  {
    id: 7,
    studentId: "1234567890",
    name: "Allen Black",
    email: "allen@black.com",
    photo:
      "https://images.pexels.com/photos/1438081/pexels-photo-1438081.jpeg?auto=compress&cs=tinysrgb&w=1200",
    phone: "1234567890",
    grade: 7,
    class: "5A",
    address: "123 Main St, Anytown, USA",
    status: "Active",
  },
  {
    id: 8,
    studentId: "1234567890",
    name: "Ophelia Castro",
    email: "ophelia@castro.com",
    photo:
      "https://images.pexels.com/photos/1036623/pexels-photo-1036623.jpeg?auto=compress&cs=tinysrgb&w=1200",
    phone: "1234567890",
    grade: 8,
    class: "5A",
    address: "123 Main St, Anytown, USA",
    status: "Active",
  },
  {
    id: 9,
    studentId: "1234567890",
    name: "Derek Briggs",
    email: "derek@briggs.com",
    photo:
      "https://images.pexels.com/photos/842980/pexels-photo-842980.jpeg?auto=compress&cs=tinysrgb&w=1200",
    phone: "1234567890",
    grade: 9,
    class: "5A",
    address: "123 Main St, Anytown, USA",
    status: "Active",
  },
  {
    id: 10,
    studentId: "1234567890",
    name: "John Glover",
    email: "john@glover.com",
    photo:
      "https://images.pexels.com/photos/1043474/pexels-photo-1043474.jpeg?auto=compress&cs=tinysrgb&w=1200",
    phone: "1234567890",
    grade: 10,
    class: "5A",
    address: "123 Main St, Anytown, USA",
    status: "Active",
  },
];

export const parentsData = [
  {
    id: 1,
    name: "John Doe",
    students: ["Sarah Brewer"],
    email: "john@doe.com",
    phone: "1234567890",
    address: "123 Main St, Anytown, USA",
  },
  {
    id: 2,
    name: "Jane Doe",
    students: ["Cecilia Bradley"],
    email: "jane@doe.com",
    phone: "1234567890",
    address: "123 Main St, Anytown, USA",
  },
  {
    id: 3,
    name: "Mike Geller",
    students: ["Fanny Caldwell"],
    email: "mike@geller.com",
    phone: "1234567890",
    address: "123 Main St, Anytown, USA",
  },
  {
    id: 4,
    name: "Jay French",
    students: ["Mollie Fitzgerald", "Ian Bryant"],
    email: "mike@geller.com",
    phone: "1234567890",
    address: "123 Main St, Anytown, USA",
  },
  {
    id: 5,
    name: "Jane Smith",
    students: ["Mable Harvey"],
    email: "mike@geller.com",
    phone: "1234567890",
    address: "123 Main St, Anytown, USA",
  },
  {
    id: 6,
    name: "Anna Santiago",
    students: ["Joel Lambert"],
    email: "mike@geller.com",
    phone: "1234567890",
    address: "123 Main St, Anytown, USA",
  },
  {
    id: 7,
    name: "Allen Black",
    students: ["Carrie Tucker", "Lilly Underwood"],
    email: "mike@geller.com",
    phone: "1234567890",
    address: "123 Main St, Anytown, USA",
  },
  {
    id: 8,
    name: "Ophelia Castro",
    students: ["Alexander Blair"],
    email: "mike@geller.com",
    phone: "1234567890",
    address: "123 Main St, Anytown, USA",
  },
  {
    id: 9,
    name: "Derek Briggs",
    students: ["Susan Webster", "Maude Stone"],
    email: "mike@geller.com",
    phone: "1234567890",
    address: "123 Main St, Anytown, USA",
  },
  {
    id: 10,
    name: "John Glover",
    students: ["Stella Scott"],
    email: "mike@geller.com",
    phone: "1234567890",
    address: "123 Main St, Anytown, USA",
  },
];

export const subjectsData = [
  {
    id: 1,
    name: "Math",
    teachers: ["Alice Phelps", "Russell Davidson"],
  },
  {
    id: 2,
    name: "English",
    teachers: ["Manuel Becker", "Eddie Chavez"],
  },
  {
    id: 3,
    name: "Physics",
    teachers: ["Lola Newman", "Darrell Delgado"],
  },
  {
    id: 4,
    name: "Chemistry",
    teachers: ["Nathan Kelly", "Benjamin Snyder"],
  },
  {
    id: 5,
    name: "Biology",
    teachers: ["Alma Benson", "Lina Collier"],
  },
  {
    id: 6,
    name: "History",
    teachers: ["Hannah Bowman", "Betty Obrien"],
  },
  {
    id: 7,
    name: "Geography",
    teachers: ["Lora French", "Sue Brady"],
  },
  {
    id: 8,
    name: "Art",
    teachers: ["Harriet Alvarado", "Mayme Keller"],
  },
  {
    id: 9,
    name: "Music",
    teachers: ["Gertrude Roy", "Rosa Singleton"],
  },
  {
    id: 10,
    name: "Literature",
    teachers: ["Effie Lynch", "Brett Flowers"],
  },
];

export const classesData = [
  {
    id: 1,
    name: "JSS 1 A",
    capacity: 25,
    grade: 7,
    supervisor: "Joseph Padilla",
  },
  {
    id: 2,
    name: "JSS 1 B",
    capacity: 25,
    grade: 7,
    supervisor: "Blake Joseph",
  },
  {
    id: 3,
    name: "JSS 1 C",
    capacity: 25,
    grade: 7,
    supervisor: "Tom Bennett",
  },
  {
    id: 4,
    name: "JSS 2 A",
    capacity: 25,
    grade: 8,
    supervisor: "Aaron Collins",
  },
  {
    id: 5,
    name: "JSS 2 B",
    capacity: 25,
    grade: 8,
    supervisor: "Iva Frank",
  },
  {
    id: 6,
    name: "JSS 2 C",
    capacity: 25,
    grade: 8,
    supervisor: "Leila Santos",
  },
  {
    id: 7,
    name: "JSS 3 A",
    capacity: 25,
    grade: 9,
    supervisor: "Carrie Walton",
  },
  {
    id: 8,
    name: "JSS 3 B",
    capacity: 25,
    grade: 9,
    supervisor: "Christopher Butler",
  },
  {
    id: 9,
    name: "JSS 3 C",
    capacity: 25,
    grade: 9,
    supervisor: "Marc Miller",
  },
  {
    id: 10,
    name: "SS 1 A",
    capacity: 25,
    grade: 10,
    supervisor: "Ophelia Marsh",
  },
  {
    id: 11,
    name: "SS 1 B",
    capacity: 25,
    grade: 10,
    supervisor: "Roger Henderson",
  },
  {
    id: 12,
    name: "SS 1 C",
    capacity: 25,
    grade: 10,
    supervisor: "Steven Ward",
  },
  {
    id: 13,
    name: "SS 2 A",
    capacity: 25,
    grade: 11,
    supervisor: "Jeanette Banks",
  },
  {
    id: 14,
    name: "SS 2 B",
    capacity: 25,
    grade: 11,
    supervisor: "Lula Lloyd",
  },
  {
    id: 15,
    name: "SS 2 C",
    capacity: 25,
    grade: 11,
    supervisor: "Myra Foster",
  },
  {
    id: 16,
    name: "SS 3 A",
    capacity: 25,
    grade: 12,
    supervisor: "Trevor Yates",
  },
  {
    id: 17,
    name: "SS 3 B",
    capacity: 25,
    grade: 12,
    supervisor: "Lillie Fitzgerald",
  },
  {
    id: 18,
    name: "SS 3 C",
    capacity: 25,
    grade: 12,
    supervisor: "Ola Bridges",
  },
];

export const lessonsData = [
  {
    id: 1,
    subject: "Math",
    class: "1A",
    teacher: "Tommy Wise",
  },
  {
    id: 2,
    subject: "English",
    class: "2A",
    teacher: "Rhoda Frank",
  },
  {
    id: 3,
    subject: "Science",
    class: "3A",
    teacher: "Della Dunn",
  },
  {
    id: 4,
    subject: "Social Studies",
    class: "1B",
    teacher: "Bruce Rodriguez",
  },
  {
    id: 5,
    subject: "Art",
    class: "4A",
    teacher: "Birdie Butler",
  },
  {
    id: 6,
    subject: "Music",
    class: "5A",
    teacher: "Bettie Oliver",
  },
  {
    id: 7,
    subject: "History",
    class: "6A",
    teacher: "Herman Howard",
  },
  {
    id: 8,
    subject: "Geography",
    class: "6B",
    teacher: "Lucinda Thomas",
  },
  {
    id: 9,
    subject: "Physics",
    class: "6C",
    teacher: "Ronald Roberts",
  },
  {
    id: 10,
    subject: "Chemistry",
    class: "4B",
    teacher: "Julia Pittman",
  },
];

export const examsData = [
  {
    id: 1,
    subject: "Math",
    class: "1A",
    teacher: "Martha Morris",
    date: "2025-01-01",
  },
  {
    id: 2,
    subject: "English",
    class: "2A",
    teacher: "Randall Garcia",
    date: "2025-01-01",
  },
  {
    id: 3,
    subject: "Science",
    class: "3A",
    teacher: "Myrtie Scott",
    date: "2025-01-01",
  },
  {
    id: 4,
    subject: "Social Studies",
    class: "1B",
    teacher: "Alvin Swanson",
    date: "2025-01-01",
  },
  {
    id: 5,
    subject: "Art",
    class: "4A",
    teacher: "Mabelle Wallace",
    date: "2025-01-01",
  },
  {
    id: 6,
    subject: "Music",
    class: "5A",
    teacher: "Dale Thompson",
    date: "2025-01-01",
  },
  {
    id: 7,
    subject: "History",
    class: "6A",
    teacher: "Allie Conner",
    date: "2025-01-01",
  },
  {
    id: 8,
    subject: "Geography",
    class: "6B",
    teacher: "Hunter Fuller",
    date: "2025-01-01",
  },
  {
    id: 9,
    subject: "Physics",
    class: "7A",
    teacher: "Lois Lindsey",
    date: "2025-01-01",
  },
  {
    id: 10,
    subject: "Chemistry",
    class: "8A",
    teacher: "Vera Soto",
    date: "2025-01-01",
  },
];

export const assignmentsData = [
  {
    id: 1,
    subject: "Math",
    class: "1A",
    teacher: "Anthony Boone",
    dueDate: "2025-01-01",
  },
  {
    id: 2,
    subject: "English",
    class: "2A",
    teacher: "Clifford Bowen",
    dueDate: "2025-01-01",
  },
  {
    id: 3,
    subject: "Science",
    class: "3A",
    teacher: "Catherine Malone",
    dueDate: "2025-01-01",
  },
  {
    id: 4,
    subject: "Social Studies",
    class: "1B",
    teacher: "Willie Medina",
    dueDate: "2025-01-01",
  },
  {
    id: 5,
    subject: "Art",
    class: "4A",
    teacher: "Jose Ruiz",
    dueDate: "2025-01-01",
  },
  {
    id: 6,
    subject: "Music",
    class: "5A",
    teacher: "Katharine Owens",
    dueDate: "2025-01-01",
  },
  {
    id: 7,
    subject: "History",
    class: "6A",
    teacher: "Shawn Norman",
    dueDate: "2025-01-01",
  },
  {
    id: 8,
    subject: "Geography",
    class: "6B",
    teacher: "Don Holloway",
    dueDate: "2025-01-01",
  },
  {
    id: 9,
    subject: "Physics",
    class: "7A",
    teacher: "Franklin Gregory",
    dueDate: "2025-01-01",
  },
  {
    id: 10,
    subject: "Chemistry",
    class: "8A",
    teacher: "Danny Nguyen",
    dueDate: "2025-01-01",
  },
];

export const resultsData = [
  {
    id: 1,
    subject: "Math",
    class: "1A",
    teacher: "John Doe",
    student: "John Doe",
    date: "2025-01-01",
    type: "exam",
    score: 90,
  },
  {
    id: 2,
    subject: "English",
    class: "2A",
    teacher: "John Doe",
    student: "John Doe",
    date: "2025-01-01",
    type: "exam",
    score: 90,
  },
  {
    id: 3,
    subject: "Science",
    class: "3A",
    teacher: "John Doe",
    student: "John Doe",
    date: "2025-01-01",
    type: "exam",
    score: 90,
  },
  {
    id: 4,
    subject: "Social Studies",
    class: "1B",
    teacher: "John Doe",
    student: "John Doe",
    date: "2025-01-01",
    type: "exam",
    score: 90,
  },
  {
    id: 5,
    subject: "Art",
    class: "4A",
    teacher: "John Doe",
    student: "John Doe",
    date: "2025-01-01",
    type: "exam",
    score: 90,
  },
  {
    id: 6,
    subject: "Music",
    class: "5A",
    teacher: "John Doe",
    student: "John Doe",
    date: "2025-01-01",
    type: "exam",
    score: 90,
  },
  {
    id: 7,
    subject: "History",
    class: "6A",
    teacher: "John Doe",
    student: "John Doe",
    date: "2025-01-01",
    type: "exam",
    score: 90,
  },
  {
    id: 8,
    subject: "Geography",
    class: "6B",
    teacher: "John Doe",
    student: "John Doe",
    date: "2025-01-01",
    type: "exam",
    score: 90,
  },
  {
    id: 9,
    subject: "Physics",
    class: "7A",
    teacher: "John Doe",
    student: "John Doe",
    date: "2025-01-01",
    type: "exam",
    score: 90,
  },
  {
    id: 10,
    subject: "Chemistry",
    class: "8A",
    teacher: "John Doe",
    student: "John Doe",
    date: "2025-01-01",
    type: "exam",
    score: 90,
  },
];

export const eventsData = [
  {
    id: 1,
    title: "Lake Trip",
    class: "1A",
    date: "2025-01-01",
    startTime: "10:00",
    endTime: "11:00",
    visibleTo: ["admin", "teacher", "student", "parent"],
    description: "Annual school trip to the local lake for nature study and recreation.",
  },
  {
    id: 2,
    title: "Staff Strategy Meeting",
    class: "N/A",
    date: "2025-01-05",
    startTime: "14:00",
    endTime: "16:00",
    visibleTo: ["admin", "teacher"],
    description: "Important meeting for all staff members to discuss the term's academic strategy.",
  },
  {
    id: 3,
    title: "Beach Cleanup",
    class: "3A",
    date: "2025-01-10",
    startTime: "08:00",
    endTime: "12:00",
    visibleTo: ["admin", "teacher", "student", "parent"],
    description: "Community service event to clean up the local beach. Equipment provided.",
  },
  {
    id: 4,
    title: "Admin Audit",
    class: "N/A",
    date: "2025-01-15",
    startTime: "09:00",
    endTime: "17:00",
    visibleTo: ["admin"],
    description: "Internal administrative audit for the first half of the academic session.",
  },
  {
    id: 5,
    title: "Music Concert",
    class: "5A",
    date: "2025-01-20",
    startTime: "18:00",
    endTime: "20:00",
    visibleTo: ["admin", "teacher", "student", "parent"],
    description: "Spring music concert featuring the school choir and orchestra.",
  },
  {
    id: 6,
    title: "Magician Show",
    class: "1B",
    date: "2025-01-25",
    startTime: "11:00",
    endTime: "12:00",
    visibleTo: ["admin", "teacher", "student", "parent"],
    description: "Entertaining magic show for junior secondary students.",
  },
  {
    id: 7,
    title: "Parent-Teacher Conference",
    class: "All",
    date: "2025-02-01",
    startTime: "09:00",
    endTime: "15:00",
    visibleTo: ["admin", "teacher", "parent"],
    description: "Termly meeting between parents and teachers to discuss student progress.",
  },
  {
    id: 8,
    title: "Cycling Race",
    class: "3B",
    date: "2025-02-05",
    startTime: "07:00",
    endTime: "10:00",
    visibleTo: ["admin", "teacher", "student", "parent"],
    description: "Inter-class cycling competition held at the school sports complex.",
  },
  {
    id: 9,
    title: "Art Exhibition",
    class: "4B",
    date: "2025-02-10",
    startTime: "10:00",
    endTime: "16:00",
    visibleTo: ["admin", "teacher", "student", "parent"],
    description: "Public exhibition showcasing the creative works of our visual arts students.",
  },
  {
    id: 10,
    title: "Sports Tournament",
    class: "5B",
    date: "2025-02-15",
    startTime: "08:00",
    endTime: "18:00",
    visibleTo: ["admin", "teacher", "student", "parent"],
    description: "Regional inter-school sports tournament hosted on our grounds.",
  },
];

export const announcementsData = [
  {
    id: 1,
    title: "About 4A Math Test",
    class: "4A",
    date: "2025-01-01",
    visibleTo: ["admin", "teacher", "student", "parent"],
    description: "The Math test for JSS 1A has been rescheduled to next Friday. Please review chapters 3 and 4.",
  },
  {
    id: 2,
    title: "New Curriculum Guidelines",
    class: "N/A",
    date: "2025-01-05",
    visibleTo: ["admin", "teacher"],
    description: "New guidelines for the mathematics curriculum have been uploaded to the staff portal.",
  },
  {
    id: 3,
    title: "About 3B Math Test",
    class: "3B",
    date: "2025-01-10",
    visibleTo: ["admin", "teacher", "student", "parent"],
    description: "Results for the 3B Math test are now available for viewing in the student portal.",
  },
  {
    id: 4,
    title: "Annual Budget Review",
    class: "N/A",
    date: "2025-01-15",
    visibleTo: ["admin"],
    description: "The annual budget review meeting is scheduled for Monday at 10:00 AM in the boardroom.",
  },
  {
    id: 5,
    title: "About 8C Math Test",
    class: "8C",
    date: "2025-01-20",
    visibleTo: ["admin", "teacher", "student", "parent"],
    description: "Please note the change in venue for the 8C Math test. It will now be held in Hall B.",
  },
  {
    id: 6,
    title: "Extracurricular Activities",
    class: "All",
    date: "2025-01-25",
    visibleTo: ["admin", "teacher", "student", "parent"],
    description: "Sign-ups for this term's extracurricular activities are now open. Visit the sports office.",
  },
  {
    id: 7,
    title: "Staff Welfare Update",
    class: "N/A",
    date: "2025-02-01",
    visibleTo: ["admin", "teacher"],
    description: "Updated information regarding staff health insurance benefits is now available in the HR office.",
  },
  {
    id: 8,
    title: "About 4B Math Test",
    class: "4B",
    date: "2025-02-05",
    visibleTo: ["admin", "teacher", "student", "parent"],
    description: "A revision session for the 4B Math test will be held this Thursday during lunch break.",
  },
  {
    id: 9,
    title: "Cafeteria Menu Change",
    class: "All",
    date: "2025-02-10",
    visibleTo: ["admin", "teacher", "student", "parent"],
    description: "Due to popular demand, we are adding more healthy options to the daily cafeteria menu.",
  },
  {
    id: 10,
    title: "Exam Registry Opening",
    class: "Admin Only",
    date: "2025-02-15",
    visibleTo: ["admin"],
    description: "The registry for final year examinations is now open for administrative processing.",
  },
];

// YOU SHOULD CHANGE THE DATES OF THE EVENTS TO THE CURRENT DATE TO SEE THE EVENTS ON THE CALENDAR
export const calendarEvents = [
  {
    title: "Math",
    allDay: false,
    start: new Date(2025, 4, 21, 8, 0),
    end: new Date(2025, 4, 21, 8, 45),
  },
  {
    title: "English",
    allDay: false,
    start: new Date(2025, 4, 21, 9, 0),
    end: new Date(2025, 4, 21, 9, 45),
  },
  {
    title: "Biology",
    allDay: false,
    start: new Date(2025, 4, 12, 10, 0),
    end: new Date(2025, 4, 12, 10, 45),
  },
  {
    title: "Physics",
    allDay: false,
    start: new Date(2025, 4, 12, 11, 0),
    end: new Date(2025, 4, 12, 11, 45),
  },
  {
    title: "Chemistry",
    allDay: false,
    start: new Date(2025, 4, 12, 13, 0),
    end: new Date(2025, 4, 12, 13, 45),
  },
  {
    title: "History",
    allDay: false,
    start: new Date(2025, 4, 12, 14, 0),
    end: new Date(2025, 4, 12, 14, 45),
  },
  {
    title: "English",
    allDay: false,
    start: new Date(2025, 4, 13, 9, 0),
    end: new Date(2025, 4, 13, 9, 45),
  },
  {
    title: "Biology",
    allDay: false,
    start: new Date(2025, 4, 13, 10, 0),
    end: new Date(2025, 4, 13, 10, 45),
  },
  {
    title: "Physics",
    allDay: false,
    start: new Date(2025, 4, 13, 11, 0),
    end: new Date(2025, 4, 13, 11, 45),
  },

  {
    title: "History",
    allDay: false,
    start: new Date(2025, 4, 13, 14, 0),
    end: new Date(2025, 4, 13, 14, 45),
  },
  {
    title: "Math",
    allDay: false,
    start: new Date(2025, 4, 14, 8, 0),
    end: new Date(2025, 4, 14, 8, 45),
  },
  {
    title: "Biology",
    allDay: false,
    start: new Date(2025, 4, 14, 10, 0),
    end: new Date(2025, 4, 14, 10, 45),
  },

  {
    title: "Chemistry",
    allDay: false,
    start: new Date(2025, 4, 14, 13, 0),
    end: new Date(2025, 4, 14, 13, 45),
  },
  {
    title: "History",
    allDay: false,
    start: new Date(2025, 4, 14, 14, 0),
    end: new Date(2025, 4, 14, 14, 45),
  },
  {
    title: "English",
    allDay: false,
    start: new Date(2025, 4, 15, 9, 0),
    end: new Date(2025, 4, 15, 9, 45),
  },
  {
    title: "Biology",
    allDay: false,
    start: new Date(2025, 4, 15, 10, 0),
    end: new Date(2025, 4, 15, 10, 45),
  },
  {
    title: "Physics",
    allDay: false,
    start: new Date(2025, 4, 15, 11, 0),
    end: new Date(2025, 4, 15, 11, 45),
  },

  {
    title: "History",
    allDay: false,
    start: new Date(2025, 4, 15, 14, 0),
    end: new Date(2025, 4, 15, 14, 45),
  },
  {
    title: "Math",
    allDay: false,
    start: new Date(2025, 4, 16, 8, 0),
    end: new Date(2025, 4, 16, 8, 45),
  },
  {
    title: "English",
    allDay: false,
    start: new Date(2025, 4, 16, 9, 0),
    end: new Date(2025, 4, 16, 9, 45),
  },

  {
    title: "Physics",
    allDay: false,
    start: new Date(2025, 4, 16, 11, 0),
    end: new Date(2025, 4, 16, 11, 45),
  },
  {
    title: "Chemistry",
    allDay: false,
    start: new Date(2025, 4, 16, 13, 0),
    end: new Date(2025, 4, 16, 13, 45),
  },
  {
    title: "History",
    allDay: false,
    start: new Date(2025, 4, 16, 14, 0),
    end: new Date(2025, 4, 16, 14, 45),
  },
];

export const feeStructureData = [
  {
    grade: 7,
    label: "JSS 1",
    tuition: 500,
    bus: 100,
    uniform: 50,
    other: 20,
  },
  {
    grade: 8,
    label: "JSS 2",
    tuition: 550,
    bus: 100,
    uniform: 50,
    other: 20,
  },
  {
    grade: 9,
    label: "JSS 3",
    tuition: 600,
    bus: 100,
    uniform: 50,
    other: 20,
  },
  {
    grade: 10,
    label: "SS 1",
    tuition: 700,
    bus: 120,
    uniform: 60,
    other: 30,
  },
  {
    grade: 11,
    label: "SS 2",
    tuition: 750,
    bus: 120,
    uniform: 60,
    other: 30,
  },
  {
    grade: 12,
    label: "SS 3",
    tuition: 800,
    bus: 120,
    uniform: 60,
    other: 30,
  },
];

export const studentPaymentsData = [
  {
    id: 1,
    studentId: "1234567890",
    name: "John Doe",
    grade: 7,
    class: "1B",
    totalDue: 670,
    amountPaid: 400,
    balance: 270,
    status: "Partial",
  },
  {
    id: 2,
    studentId: "1234567890",
    name: "Jane Doe",
    grade: 8,
    class: "5A",
    totalDue: 720,
    amountPaid: 720,
    balance: 0,
    status: "Paid",
  },
  {
    id: 3,
    studentId: "1234567890",
    name: "Mike Geller",
    grade: 9,
    class: "5A",
    totalDue: 770,
    amountPaid: 0,
    balance: 770,
    status: "Unpaid",
  },
  {
    id: 4,
    studentId: "1234567890",
    name: "Jay French",
    grade: 10,
    class: "5A",
    totalDue: 910,
    amountPaid: 500,
    balance: 410,
    status: "Partial",
  },
  {
    id: 5,
    studentId: "1234567890",
    name: "Jane Smith",
    grade: 11,
    class: "5A",
    totalDue: 960,
    amountPaid: 960,
    balance: 0,
    status: "Paid",
  },
  {
    id: 6,
    studentId: "1234567890",
    name: "Anna Santiago",
    grade: 12,
    class: "5A",
    totalDue: 1010,
    amountPaid: 1010,
    balance: 0,
    status: "Paid",
  },
  {
    id: 7,
    studentId: "1234567890",
    name: "Allen Black",
    grade: 7,
    class: "5A",
    totalDue: 670,
    amountPaid: 670,
    balance: 0,
    status: "Paid",
  },
  {
    id: 8,
    studentId: "1234567890",
    name: "Ophelia Castro",
    grade: 8,
    class: "5A",
    totalDue: 720,
    amountPaid: 300,
    balance: 420,
    status: "Partial",
  },
  {
    id: 9,
    studentId: "1234567890",
    name: "Derek Briggs",
    grade: 9,
    class: "5A",
    totalDue: 770,
    amountPaid: 770,
    balance: 0,
    status: "Paid",
  },
  {
    id: 10,
    studentId: "1234567890",
    name: "John Glover",
    grade: 10,
    class: "5A",
    totalDue: 910,
    amountPaid: 0,
    balance: 910,
    status: "Unpaid",
  },
];

export const currentAcademicCycle = {
  session: "2025/2026",
  term: "First Term",
};
